...
from sahara.plugins.hbase.hadoop2 import config_helper as c_helper
from sahara.plugins.hbase.hadoop2 import oozie_helper as o_helper
from sahara.plugins.hbase import utils as vu
...
ZK_CONF_DIR = '/opt/zookeeper/zookeeper/conf'
...
def _get_hadoop_configs(pctx, instance):
    ...
    hbase_master = vu.get_instance_hostname(vu.get_hbasemaster(cluster))
    zk_nodes = vu.get_zkserver(cluster)
    zk_server = [inst.fqdn() for inst in zk_nodes]
    if hbase_master:
        hbase_cfg = {
            'hbase.rootdir': 'hdfs://%s:9000/hbase' % hbase_master,
            'hbase.cluster.distributed': True,
            'hbase.zookeeper.property.dataDir': ZK_DATA_DIR,
            'hbase.zookeeper.quorum': ','.join(zk_server)
        }

        confs['HBase'] = hbase_cfg

    _addtional_configs(cluster, [])

    return confs


def _dispatch_addtional_hbase_config(cluster, instances):
    hbase_master = vu.get_instance_hostname(vu.get_hbasemaster(cluster))
    zk_nodes = [instance for instance in vu.get_zkserver(cluster)
                if instance not in instances]
    zk_server = [instance.fqdn() for instance in zk_nodes]
    confs = {}
    if hbase_master:
        hbase_cfg = {
            'hbase.rootdir': 'hdfs://%s:9000/hbase' % hbase_master,
        ...
        instances = utils.get_instances(cluster)
        for instance in instances:
            _push_xml_configs(instance, confs)


def _addtional_configs(cluster, instances):
    zk_nodes = [instance for instance in vu.get_zkserver(cluster)
                if instance not in instances]
    if len(zk_nodes) > 0:
        zk_cfg = [
        ...
        for index, instance in enumerate(zk_nodes):
            zk_cfg.append('server.%s=%s:2888:3888' %
                          (str(index), instance.fqdn()))
            # we should keep the number in zoo.cfg as myid
            my_id = {
                '%s/myid' % ZK_DATA_DIR: str(index)
            }
            with instance.remote() as r:
                r.write_files_to(my_id, run_as_root=True)

        for instance in zk_nodes:
            with instance.remote() as r:
                r.write_file_to('%s/zoo.cfg' % ZK_CONF_DIR,
                                '\n'.join(zk_cfg), run_as_root=True)

    hbase_master = vu.get_instance_hostname(vu.get_hbasemaster(cluster))
    rg_server = [instance.fqdn()
                 for instance in vu.get_regionserver(cluster)
                 if instance not in instances]
    if hbase_master:
        with vu.get_hbasemaster(cluster).remote() as r:
            r.write_file_to('%s/regionservers' % HBASE_CONF_DIR,
                            '\n'.join(rg_server), run_as_root=True)
...
def _push_xml_configs(instance, configs):
    ...
    service_to_conf_map = {
        ...
        'HBase': '%s/hbase-site.xml' % HBASE_CONF_DIR
    }
    xml_confs = {}
...
def _post_configuration(pctx, instance):
    ...
    post_conf_script = f.get_file_text(
        'plugins/hbase/hadoop2/resources/post_conf.template')
    ...
    with instance.remote() as r:
        ...
        if c_helper.is_data_locality_enabled(pctx,
            ...
            r.write_file_to(t_script, f.get_file_text(
                            'plugins/hbase/hadoop2/resources/topology.sh'),
                            run_as_root=True)
