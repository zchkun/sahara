...
from sahara.plugins.hbase.hadoop2 import config
...
from sahara.plugins.hbase.hadoop2 import utils as pu
...
def scale_cluster(pctx, cluster, instances):
    ...
    zk = vu.get_zkserver(cluster)
    if zk:
        run.start_zookeeper(zk)
    hb = vu.get_hbasemaster(cluster)
    if hb:
        run.start_hbaseserver(hb)
...
def decommission_nodes(pctx, cluster, instances):
    ...
    config._addtional_configs(cluster, instances)
    config._dispatch_addtional_hbase_config(cluster, instances)
    config.configure_topology_data(pctx, cluster)
