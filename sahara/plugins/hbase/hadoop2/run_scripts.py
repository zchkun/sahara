    ...
    with context.ThreadGroup() as tg:
        for instance in instances:
            with context.set_current_instance_id(instance.instance_id):
                ...
                tg.spawn('hbase-start-processes-%s' % instance.instance_name,
...
@cpo.event_wrapper(True, step=pu.start_process_event_message("ZooKeeper"))
def start_zookeeper(instances):
    for instance in instances:
        instance.remote().execute_command(
            'sudo su - -c '
            '"/opt/zookeeper/zookeeper/bin/zkServer.sh start" hadoop')


@cpo.event_wrapper(True, step=pu.start_process_event_message("HBase"))
def start_hbaseserver(instance):
    instance.remote().execute_command(
        'sudo su - -c "/opt/hbase/bin/start-hbase.sh" hadoop')
        ...
        with instance.remote() as r:
            if c_helper.is_mysql_enabled(pctx, instance.cluster):
                ...
                sql_script = files.get_file_text(
                    'plugins/hbase/hadoop2/resources/create_oozie_db.sql')
    ...
    with context.set_current_instance_id(instance.instance_id):
        with instance.remote() as r:
            ...
            if c_helper.is_mysql_enabled(pctx, instance.cluster):
                ...
                sql_script = files.get_file_text(
                    'plugins/hbase/hadoop2/resources/create_hive_db.sql'
                )
